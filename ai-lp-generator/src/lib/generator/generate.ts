...
export function tryParseTsxOrThrow(code: string): void {
...
export function fixUnterminatedJsxAttrStrings(input: string): string {
  ...
  const hasUnclosedAttrOnLine = (line: string): boolean => {
    const attrStartIndex = line.search(/\w+\s*=\s*"[^"]*$/)
    return attrStartIndex >= 0
  }
...
export async function generateProject(options: GenerateOptions): Promise<ProjectInfo> {
  const { 
    start = false, 
    concept = '', 
    name = '', 
    file = '', 
    skipAiFix = false,
    outputRoot = path.join(process.cwd(), 'generated_projects'),
    v0ApiKey = process.env.V0_API_KEY,
    v0Model = process.env.V0_MODEL || 'v0-1.5-md'
  } = options

  if (!concept && !name && !file) {
    throw new Error('少なくともコンセプト、name、file のいずれかを指定してください。')
  }

  if (!v0ApiKey) {
    throw new Error('V0_API_KEY が未設定です。')
  }
